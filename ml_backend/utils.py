...
def list_files_in_dataset(app, dataset_id):
    """
    app: The flask app, has s3 config store in app.config
    source_path: relative to py file
    ...
    operation_parameters = {'Bucket': app.config['S3_BUCKET'],
                            'Prefix': prefix}
...
def fetch_image(app, s3_source_path):
    """
    app: The flask app, has s3 config store in app.config
    source_path: relative to py file
    ...
    img = load_img(io.BytesIO(response['Body'].read()))
    # img.show()
    return img